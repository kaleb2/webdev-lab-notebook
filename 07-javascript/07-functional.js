...
const mapNameFamily = (characters) => {
  let nameFam = characters.map((c) => `${c.name} of House ${c.house}`);
  return nameFam;
};

const filterFamily = (characters, house) => {
  let houseList = characters.filter((c) => c.house === house);
  return houseList;
};

const reduceHouses = (characters) => {
  let houseCounts = characters.reduce((accumulator, current) => {
